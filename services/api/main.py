...
from database import get_db, User, Shoot, Asset, Job, JobEvent, Credit, JobStatus
import os
import uuid
import shutil
...
def create_job(
    ...
    event = JobEvent(
        job_id=job.id,
        event_type="created",
        details=json.dumps({"prompt": prompt, "tier": tier, "credits_used": credits_used})
    )
    db.add(event)
    db.commit()
    db.refresh(job)
                ...
                "jobs": [
                    {
                        ...
                        "status": job.status.value,
                        "prompt": job.prompt,
                        "output_url": f"/outputs/{os.path.basename(job.output_path)}" if job.output_path else None,
